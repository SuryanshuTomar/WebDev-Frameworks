...
export const studentSlice = createSlice({
	...
	reducers: {
		...
		updateStudent: (state, action) => {
			const { id, ...studentPayload } = action.payload;

			// Format for updating data using studentAdapter
			// studentAdapter.updateOne(state, { id, changes: payload });

			const updatedStudent = {
				id,
				changes: { ...studentPayload },
			};
			studentAdapter.updateOne(state, updatedStudent);
		},
